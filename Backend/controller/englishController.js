...
const {EnglishQuestions} = require("../DataQuestions/English")
const English = require("../models/EnglishSchema");
...
const createQuestion =async (req,res) =>{
  ...
  //  })
  //}
  const iscreate =await English.create(EnglishQuestions)
  if(!iscreate){
   return res.json({
...
const deleteQuestion =async (req,res) =>{
  ...
  const isFound=await English.findOne({questionId});
  if(!isFound){
   return res.json({
        msg:"Please question was not found"
    })
  }
 const isDone = await English.deleteOne({isFound})
  if(!isDone){
    return res.json({
